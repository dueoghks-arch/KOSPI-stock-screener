...
def send_email(content, is_html=False):
    ...
    msg['Subject'] = f"📈 [국장 스캐너] 트리플 AND(이평선 돌파 + 52주 신고가) 포착 리포트 ({datetime.now().strftime('%Y-%m-%d')})"
    msg['From'] = user
    msg['To'] = user
...
def screen_krx_stocks():
    ...
    results = []
    print(f"📊 [SCAN] Analyzing {len(tickers)} assets under Triple AND Logic (Crossover & Proximity & 52W High)...")
    ...
    for ticker in all_close_data.columns:
        try:
            ...
            # ---------------------------------------------------------
            # 조건 3. 현재가가 52주 최고가(신고가)인지 확인 (최근 52주 종가 중 최고치)
            # ---------------------------------------------------------
